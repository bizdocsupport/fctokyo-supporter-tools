...
const isCupCompetition=r=>{
  const name=`${r.competition_name||''} ${r.competition_group||''}`;
  return name.includes('天皇杯')||name.includes('ルヴァン');
};
function calendarSpec(r){
  // 天皇杯・ルヴァンカップはHOME/AWAYを問わず一般発売日時で登録する。
  if(isCupCompetition(r)&&hasConfirmedTime(r.general_at)){
    return{
      start:parseDate(r.general_at),
      titlePrefix:'【チケット一般発売】',
      detailTitle:`FC東京 ${r.competition_name||r.competition_group||'カップ戦'} チケット一般発売`
    };
  }
...
function cardsHtml(rows){return rows.map(r=>{const home=r.side==='HOME',official=safeUrl(r.ticket_source_url||r.match_url),cal=calendarUrl(r),useSocio=!isCupCompetition(r)&&isMufgNationalHome(r)&&hasConfirmedTime(r.socio_at),saleLabel=useSocio?'SOCIO':'一般発売',saleValue=useSocio?r.socio_at:r.general_at;return`<article class="match-card ${home?'home':'away'}"><div class="match-card-top"><span class="side-badge ${home?'home':'away'}">${home?'H':'A'}</span><span class="match-card-opponent">${esc(r.opponent||'未定')}</span><span class="match-card-date">${esc(matchDate(r))}</span></div><div class="match-card-details"><div class="match-card-detail"><small>${saleLabel}</small><strong class="${(!home||isCupCompetition(r))&&parseDate(r.general_at)?'sale-away':''}">${esc(formatSale(saleValue))}</strong></div><div class="match-card-detail"><small>会場</small><strong>${esc(r.stadium||'未定')}</strong></div></div><div class="match-card-actions">${official?`<a href="${esc(official)}" target="_blank" rel="noreferrer">公式情報 ↗</a>`:''}${cal?`<a class="calendar-link" href="${esc(cal)}" target="_blank" rel="noreferrer">カレンダーに追加</a>`:''}</div></article>`}).join('')}
